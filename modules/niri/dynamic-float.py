...
them afterward (like Firefox extensions). This script is like open-float for
those windows.
...
def float(id: int):
    # Find actions at: https://niri-wm.github.io/niri/niri_ipc/enum.Action.html
    send({"Action": {"MoveWindowToFloating": {"id": id}}})
    send({"Action": {"SetWindowWidth": {"id": id, "change": {"SetProportion": 25}}}})
    send({"Action": {"SetWindowHeight": {"id": id, "change": {"SetProportion": 75}}}})
    send(
        {
            "Action": {
                "MoveFloatingWindow": {
                    "id": id,
                    "x": {"SetProportion": 37.5},
                    "y": {"SetProportion": 12.5},
                }
            }
        }
    )
